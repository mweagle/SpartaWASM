...
/*
As part of the normal buid pipeline, compile the WASM file that backs the
site. We'll do this by calling back out to the mage target
*/
//go:generate mage -v BuildWASM
...
func spartaWASMLambdaFunctions(api *sparta.API) []*sparta.LambdaAWSInfo {
...
func main() {
	...
	apiGateway := sparta.NewAPIGateway("SpartaWASM", apiStage)
	...
	// Deploy it
	stackName := spartaCF.UserScopedStackName("SpartaWASM")
	sparta.MainEx(stackName,
		fmt.Sprintf("SpartaWASM provisions a static S3 hosted website running a WASM application with an API Gateway resource backed by a custom Lambda function"),
		spartaWASMLambdaFunctions(apiGateway),
		apiGateway,
		s3Site,
		nil,
		false)
